...
DEMO_USERS = [
    # (id, email, full_name, role, location, age, job)
    (1, "alice@example.com", "Alice Nguyen", "buyer", "Ho Chi Minh City", 28, "Software Engineer"),
    (2, "bob@example.com",   "Bob Tran",     "buyer", "Hanoi",            35, "Business Owner"),
    (3, "carol@example.com", "Carol Pham",   "buyer", "Da Nang",          24, "Student"),
    (4, "david@example.com", "David Le",     "admin", "Ho Chi Minh City", 42, "IT Manager"),
    (5, "emma@example.com",  "Emma Hoang",   "buyer", "Hue",              30, "Fashion Designer"),
]
ANON_ID = 6
...
def main() -> None:
    ...
    with open(OUT_FILE, "w", encoding="utf-8") as out:
        ...
        out.write(
            ...
            "-- Requires migration 000008 (location/age/job columns on users).\n\n"
            "BEGIN;\n\n"
        ...
        out.write(
            "INSERT INTO users"
            " (id, email, password_hash, full_name, role, location, age, job, created_at, updated_at)"
            " VALUES\n"
        )
        demo_rows = []
        for uid, email, name, role, loc, age, job in DEMO_USERS:
            demo_rows.append(
                f"  ({uid}, '{email}', '{DUMMY_HASH}', '{_esc(name)}', '{role}', "
                f"'{loc}', {age}, '{job}', NOW() - INTERVAL '{uid} days', NOW() - INTERVAL '1 day')"
            )
        demo_rows.append(
            f"  ({ANON_ID}, 'anonymous@glowshop.example', '{DUMMY_HASH}', 'Anonymous', 'buyer', "
            f"NULL, NULL, NULL, NOW() - INTERVAL '6 days', NOW() - INTERVAL '1 day')"
        )
        out.write(",\n".join(demo_rows) + ";\n\n")
        ...
        for b_start in range(0, len(rows), BATCH):
            ...
            out.write(
                ...
                " ai_label, final_label, created_at, updated_at) VALUES\n"
            )
            rev_rows = []
